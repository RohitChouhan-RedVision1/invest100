...
export default function Page() {
    ...
    useEffect(() => {
        const calculateHousePlan = async () => {
            try {
                const res = await axios.get(`${process.env.NEXT_PUBLIC_DATA_API}/api/calculators/house-calculator?currentHouseCost=${totalInvestment}&planHouseInYears=${investmentDuration}&expectedReturn=${expectedReturn}&inflationRate=${inflationRate}&apikey=${process.env.NEXT_PUBLIC_API_KEY}`);
                if (res.status === 200) {
                    const data = res.data
