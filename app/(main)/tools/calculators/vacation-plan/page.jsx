...
export default function Page() {
    ...
    const calculateVacationPlan = async () => {
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_DATA_API}/api/calculators/vacation-plan?planHolidayInYears=${investmentDuration}&currentExpense=${totalInvestment}&expectedReturn=${expectedReturn}&inflationRate=${inflationRate}&apikey=${process.env.NEXT_PUBLIC_API_KEY}`);
            if (res.status === 200) {
                const data = res.data
                       ...
                       <div className="grid lg:grid-cols-2 grid-cols-1 gap-4 mb-4">
                           ...
                           <div className='col-span-1'>
                               <SippieChart
                                   ...
                                   customLabels={{
                                       invested: "Current Cost of Vacation",
                                       return: "Future Cost of Vacation",
                                   }}
                               />
                               <div className="mt-5">
                                   <CalculatorReturnChart data={chartData} title={"Vacation Planning "} />
                               </div>
                           </div>
                       </div>
                   </div>
