...
const RiskProfile = () => {
    ...
    const InquiryForm = () => {
        ...
        return (
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 rounded  py-3 px-6 bg-white">
            ...
            {captchaError && (
              <p className="text-red-500 text-sm mt-2">{captchaError}</p>
                    ...
                    {/* Submit Button */}
                    <Button className="text-white border" type="submit">{!loading ? "Submit" : "Loading..."}</Button>
                </form>
            </Form>
        );
    };
